"""Card/transport error classes and the outbound-SDK error map.

These are raised by the server-to-server SDK (``classes/http.py`` etc.) when
Payme returns a JSON-RPC error. The inbound merchant webhook uses the separate
``BasePaymeException`` hierarchy in ``webhook.py``.
"""
import logging
...
class PaymeSystemError(CardError):
...
class OtpAttemptsExceededError(OtpCheckError):
    """Number of attempts to enter the code has been exceeded."""
...
class PaymeNetworkError(BaseError):
    """Network error occurred during request to the Payme server."""
...
class ReceiptsNotFoundError(BaseError):
    ...
    message = "No receipts found for the given transaction ID."
...
class UnknownPartnerError(BaseError):
    ...
    message = "Unknown partner or ID and Key not active."
