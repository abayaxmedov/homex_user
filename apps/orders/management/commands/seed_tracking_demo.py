...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        w(f"  1) Client socket:  ws://localhost:8000/ws/client/track/{order.id}/")
        w("     Header: Authorization: Bearer <CLIENT access_token>  -> 'tracking.snapshot'.")
        w(f"  2) Accept (MASTER token):   POST /api/v1/master/orders/{order.id}/accept/   body: {{}}")
        w("     -> status=accepted; client socketga 'tracking.update' (master_accepted).")
        w(f"  3) Yo'lda (MASTER token):   POST /api/v1/master/orders/{order.id}/on-way/   body: {{}}")
        w("     -> status=on_way; client socketga 'master_on_way'.")
        w(f"  4) Yetib keldi (MASTER):    POST /api/v1/master/orders/{order.id}/arrived/  body: {{}}")
        w("     -> status=arrived; client socketga 'master_arrived'.")
        w(f"  5) Yakunlash (MASTER):      POST /api/v1/master/orders/{order.id}/complete/ body: {{\"service_fee\":\"150000\",\"payment_type\":\"cash\"}}")
        w("     -> status=completed; socket yopilsa bo'ladi, to'lov/check bosqichiga o'tiladi.")
        w("  --- Har bosqichda usta lokatsiyasi (WS): ws://localhost:8000/ws/master/tracking/ + MASTER token")
        w('      Yuboring: {"lat": "41.311081", "lng": "69.240562"}  (order_id shart emas) -> client master.location oladi.')
        w("")
